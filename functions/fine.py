...
class Function():
    ...
    def parse_args(line: str) -> list:
        ...
        if not names:
            raise ArgumentError("Merci de bien vérifier que vous avez spécifié un montant ainsi qu'un destinataire pour votre amende")
        return [names]
        ...
        for member in message.guild.members:
            if to == member.display_name or to == member.nick or to == member.name:
                ...
                if not sec_account.is_registered:
                    return f"La personne à qui vous essayez de mettre une amende n'existe pas dans notre banque."
                sec_account.update_balance(-value)
                return f"L'amende d'un montant de {value} à bien été mise à {to}"
